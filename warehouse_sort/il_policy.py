...
def load_dp(checkpoint, sample_obs, action_space, device,
            obs_horizon=2, pred_horizon=32, diffusion_step_embed_dim=64,
            unet_dims=(64, 128, 256), n_groups=8, num_diffusion_iters=100,
            num_inference_steps=50, act_horizon=8):
...
def load_dp_easy(checkpoint, sample_obs, action_space, device, **kwargs):
    """Easy-tuned: act_horizon=4 was the best of {4,6,8} on a 16-episode sweep."""
    kwargs.setdefault("act_horizon", 4)
    kwargs.setdefault("pred_horizon", 32)
    return load_dp(checkpoint, sample_obs, action_space, device, **kwargs)


def load_dp_medium(checkpoint, sample_obs, action_space, device, **kwargs):
    """Medium-tuned: act_horizon=6 tied best of {4,6,8} on a 16-episode sweep."""
    kwargs.setdefault("act_horizon", 6)
    kwargs.setdefault("pred_horizon", 32)
    return load_dp(checkpoint, sample_obs, action_space, device, **kwargs)
